...
function MacLib:Window(Settings)
	...
	if getgenv().FlexyForgeGUI then getgenv().FlexyForgeGUI:Destroy() end
	local _based = Instance.new("ScreenGui")
	_based.Name = "FUCK"
	_based.Parent = gethui() or game.Players.LocalPlayer.PlayerGui
	local macLib = _based
	
	getgenv().FlexyForgeGUI = macLib
	...
	base.BackgroundTransparency = 1 -- Make base fully transparent, MainBackground will handle the color
	...
	local baseShadow = Instance.new("ImageLabel")
	baseShadow.Name = "BaseShadow"
	baseShadow.Parent = base
	baseShadow.AnchorPoint = Vector2.new(0.5, 0.5)
	baseShadow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	baseShadow.BackgroundTransparency = 1.000
	baseShadow.Position = UDim2.fromScale(0.5, 0.5)
	baseShadow.Size = UDim2.new(1, 45, 1, 45)
	baseShadow.ZIndex = -1
	baseShadow.Image = "rbxassetid://8068653048"
	baseShadow.ScaleType = Enum.ScaleType.Slice
	baseShadow.SliceCenter = Rect.new(15, 15, 175, 175)
	baseShadow.SliceScale = 1.300
	baseShadow.ImageColor3 = Settings.ShadowColor or Color3.fromRGB(0, 0, 0)

	local mainBackground = Instance.new("Frame")
	mainBackground.Name = "MainBackground"
	mainBackground.Parent = base
	mainBackground.BackgroundColor3 = Color3.fromRGB(15, 15, 15)
	mainBackground.BackgroundTransparency = Settings.AcrylicBlur and 0.05 or 0
	...
	baseUICorner.Parent = mainBackground
	...
	baseUIStroke.Parent = mainBackground
	...
	sidebar.Size = UDim2.fromScale(Settings.TabWidth or 0.325, 1)
	...
	searchBarContainer.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	searchBarContainer.BackgroundTransparency = 0.93
	searchBarContainer.BorderColor3 = Color3.fromRGB(0, 0, 0)
	searchBarContainer.BorderSizePixel = 0
	searchBarContainer.Position = UDim2.new(1, 0, 0.5, 0)
	searchBarContainer.Size = UDim2.new(0, 220, 0, 30)
	searchBarContainer.Parent = elements
	...
	searchBarStroke.Transparency = 0.88
	searchBarStroke.Parent = searchBarContainer

	local searchIconImg = Instance.new("ImageLabel")
	searchIconImg.Name = "SearchIcon"
	searchIconImg.Image = assets.searchIcon
	searchIconImg.ImageTransparency = 0.5
	...
	searchIconImg.Parent = searchBarContainer

	local searchBox = Instance.new("TextBox")
	searchBox.Name = "SearchBox"
	searchBox.FontFace = Font.new(assets.interFont, Enum.FontWeight.Regular, Enum.FontStyle.Normal)
	...
	searchBox.TextTransparency = 0.15
	...
	searchBox.Position = UDim2.new(0, 30, 0.5, 0)
	searchBox.Size = UDim2.new(1, -40, 1, 0)
	searchBox.Parent = searchBarContainer

	-- Animate search bar focus
	searchBox.Focused:Connect(function()
		Tween(searchBarContainer, TweenInfo.new(0.2, Enum.EasingStyle.Sine), {
			BackgroundTransparency = 0.88
		}):Play()
		Tween(searchBarStroke, TweenInfo.new(0.2, Enum.EasingStyle.Sine), {
			Transparency = 0.7
		}):Play()
		Tween(searchIconImg, TweenInfo.new(0.2, Enum.EasingStyle.Sine), {
	...
	interact.AnchorPoint = Vector2.new(1, 0.5)
	interact.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	interact.BackgroundTransparency = 1
	interact.BorderColor3 = Color3.fromRGB(0, 0, 0)
	interact.BorderSizePixel = 0
	interact.Position = UDim2.fromScale(1, 0.5)
	...
	searchDropdown.AnchorPoint = Vector2.new(1, 0)
	searchDropdown.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
	searchDropdown.BackgroundTransparency = 0
	searchDropdown.BorderSizePixel = 0
	searchDropdown.Position = UDim2.new(1, -20, 0, 68)
	searchDropdown.Size = UDim2.new(0, 260, 0, 0)
	searchDropdown.ZIndex = 200
	searchDropdown.Visible = false
	searchDropdown.ClipsDescendants = true
	searchDropdown.Parent = base
	...
	sdStroke.Color = Color3.fromRGB(255, 255, 255)
	sdStroke.Transparency = 0.9
	sdStroke.Parent = searchDropdown

	local sdPadding = Instance.new("UIPadding")
	sdPadding.PaddingTop = UDim.new(0, 6)
	sdPadding.PaddingBottom = UDim.new(0, 6)
	sdPadding.Parent = searchDropdown

	local sdList = Instance.new("UIListLayout")
	sdList.SortOrder = Enum.SortOrder.LayoutOrder
	sdList.Padding = UDim.new(0, 2)
	sdList.Parent = searchDropdown

	-- Scoring algorithm
	local function scoreMatch(elementName, query)
		local name = string.lower(elementName)
		local q = string.lower(query)
		if name == q then return 4 end
		if name:sub(1, #q) == q then return 3 end
		if name:find(q, 1, true) then return 2 end
		-- Fuzzy: chars of q appear in order in name
		local qi = 1
		for i = 1, #name do
			if name:sub(i, i) == q:sub(qi, qi) then
				qi += 1
	...
	local function flashElement(frame)
		local originalColor = frame.BackgroundColor3
		frame.BackgroundColor3 = Color3.fromRGB(255, 255, 255)

		-- Ensure rounded corners during flash
		local corner = frame:FindFirstChildOfClass("UICorner")
		local addedCorner = false
		if not corner then
		...
		for _ = 1, 3 do
			Tween(frame, TweenInfo.new(0.13, Enum.EasingStyle.Sine), {BackgroundTransparency = 0.82}):Play()
			task.wait(0.15)
			Tween(frame, TweenInfo.new(0.13, Enum.EasingStyle.Sine), {BackgroundTransparency = 1}):Play()
			task.wait(0.15)
		end

		frame.BackgroundColor3 = originalColor
		if addedCorner and corner then
			corner:Destroy()
		end
	end

	local function performSearch(query)
		-- Clear old rows (both result rows and "no results" label)
		for _, child in ipairs(searchDropdown:GetChildren()) do
			if not (child:IsA("UIListLayout") or child:IsA("UIPadding") or child:IsA("UICorner") or child:IsA("UIStroke")) then
				child:Destroy()
			end
		end
		...
		for _, entry in ipairs(MacLib.SearchRegistry) do
			local score = scoreMatch(entry.elementName, query)
			if score > 0 then
		...
		local maxRows = math.min(8, #results)
		if maxRows == 0 then
			-- No results row
			local noRow = Instance.new("TextLabel")
			noRow.Text = "  No results"
			noRow.TextColor3 = Color3.fromRGB(255, 255, 255)
			noRow.TextTransparency = 0.6
			noRow.TextSize = 12
			noRow.FontFace = Font.new(assets.interFont)
			noRow.TextXAlignment = Enum.TextXAlignment.Left
			noRow.BackgroundTransparency = 1
			noRow.Size = UDim2.new(1, 0, 0, 36)
			noRow.ZIndex = 201
			noRow.Parent = searchDropdown
			searchDropdown.Size = UDim2.new(0, 260, 0, 48)
		...
		for i = 1, maxRows do
			...
			nameLabel.Text = entry.elementName
			nameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
			nameLabel.TextTransparency = 0.05
			nameLabel.TextSize = 13
			nameLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium)
			...
			-- Breadcrumb (dim)
			local crumb = entry.tabName
			if entry.sectionName and entry.sectionName ~= "" then
				crumb = crumb .. " › " .. entry.sectionName
			end
			local breadLabel = Instance.new("TextLabel")
			breadLabel.Text = crumb
			breadLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
			breadLabel.TextTransparency = 0.55
			breadLabel.TextSize = 11
			breadLabel.FontFace = Font.new(assets.interFont)
			breadLabel.TextXAlignment = Enum.TextXAlignment.Left
			breadLabel.BackgroundTransparency = 1
			breadLabel.AnchorPoint = Vector2.new(0, 0)
			breadLabel.Position = UDim2.new(0, 0, 0, 26)
			breadLabel.Size = UDim2.new(1, 0, 0, 14)
			breadLabel.ZIndex = 202
			breadLabel.Parent = row

			-- Hover effect
			row.MouseEnter:Connect(function()
				Tween(row, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {BackgroundTransparency = 0.91}):Play()
			end)
			row.MouseLeave:Connect(function()
				Tween(row, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {BackgroundTransparency = 0.97}):Play()
			end)

			-- Click: switch tab, scroll if needed, flash
			local capturedEntry = entry
			row.MouseButton1Click:Connect(function()
				searchBox.Text = ""
				searchDropdown.Visible = false
				capturedEntry.tabFunctions:Select()
				task.defer(function()
					local sf = capturedEntry.elementsScrolling
					local ef = capturedEntry.elementFrame
					if sf and ef then
						-- Calculate element position within canvas
						local relY = ef.AbsolutePosition.Y - sf.AbsolutePosition.Y + sf.CanvasPosition.Y
						local viewTop = sf.CanvasPosition.Y
						local viewBot = viewTop + sf.AbsoluteSize.Y
						local elemBot = relY + ef.AbsoluteSize.Y
						-- Only scroll if element is not fully visible
						if relY < viewTop or elemBot > viewBot then
							sf.CanvasPosition = Vector2.new(0, math.max(0, relY - 20))
						end
					end
					task.spawn(flashElement, ef)
				end)
			end)
		end

		searchDropdown.Size = UDim2.new(0, 260, 0, maxRows * 48 + 14)
		searchDropdown.Visible = true
	end
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			local ap = searchDropdown.AbsolutePosition
			local as = searchDropdown.AbsoluteSize
	...
	if not Settings.DragStyle or Settings.DragStyle == 1 then
		...
		information.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				onDragStart(input)
			end
		end)
		...
		baseShadow.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				local mx, my = input.Position.X, input.Position.Y
				local px, py = mainBackground.AbsolutePosition.X, mainBackground.AbsolutePosition.Y
				local sx, sy = mainBackground.AbsoluteSize.X, mainBackground.AbsoluteSize.Y
		...
		information.InputChanged:Connect(onDragUpdate)
		windowControls.InputChanged:Connect(onDragUpdate)
		baseShadow.InputChanged:Connect(onDragUpdate)
		...
		information.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging_ = false
			end
		end)
		...
		baseShadow.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging_ = false
			end
		end)
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			local _tabName = Settings.Name or ""
			...
			function TabFunctions:Section(Settings)
				local SectionFunctions = {}
				local _sectionName = Settings.Title or Settings.Name or ""
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					buttonInteract.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Button",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = button,
					})
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					toggleName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Toggle",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = toggle,
					})
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					sliderName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Slider",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = slider,
					})
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					inputName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Input",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = input,
					})
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					keybindName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Keybind",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = keybind,
					})
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					dropdownName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Dropdown",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = dropdown,
					})
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					colorpickerName.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
					...
					table.insert(MacLib.SearchRegistry, {
						tabName = _tabName, sectionName = _sectionName,
						elementName = Settings.Name or "", elementType = "Colorpicker",
						tabFunctions = TabFunctions, elementsScrolling = elementsScrolling,
						elementFrame = colorpicker,
					})
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					headerText.TextTransparency = 0.05
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					labelText.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold)
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local configSelection = configSection:Dropdown({
					...
					Callback = function(Value)
						...
						if MacLib.AutoSave.Enabled then
							MacLib.AutoSave.Target = Value
							writefile(MacLib.Folder .. "/settings/autosave.txt", Value or "")
						end
					end,
				})
				...
				local autoSaveTargetFile = MacLib.Folder .. "/settings/autosave.txt"

				configSection:Toggle({
					Name = "Auto Save",
					Default = false,
					Callback = function(Value)
						MacLib.AutoSave.Enabled = Value
						if Value then
							local target = configSelection.Value or MacLib.CurrentConfig or MacLib.AutoSave.Target
							if not target and isfile(autoSaveTargetFile) then
								target = readfile(autoSaveTargetFile)
							end
							MacLib.AutoSave.Target = target
							if target then
								writefile(autoSaveTargetFile, target)
								WindowFunctions:Notify({
									Title = "Interface",
									Description = string.format("Auto Save enabled. Saving to %q", target),
								})
							else
								WindowFunctions:Notify({
									Title = "Interface",
									Description = "Auto Save enabled. Select a config file to save to.",
								})
							end
						else
							MacLib.AutoSave.Target = nil
						end
					end,
				}, "__AutoSaveEnabled")
				...
				-- Restore AutoSave target from file on startup
				if isfile(autoSaveTargetFile) then
					MacLib.AutoSave.Target = readfile(autoSaveTargetFile)
				end

				-- Sync dropdown selection and AutoSave target whenever a config is loaded
				table.insert(MacLib.OnConfigLoaded, function(loadedName)
					task.defer(function()
						configSelection:UpdateSelection(loadedName)
						if MacLib.AutoSave.Enabled then
							MacLib.AutoSave.Target = loadedName
							writefile(autoSaveTargetFile, loadedName)
						end
					end)
				end)
			end
	...
	BuildFolderTree() -- Ensure folders exist on startup

	function MacLib:TriggerAutoSave()
		if not MacLib.AutoSave.Enabled or MacLib.IsLoadingConfig then return end

		-- If no explicit target, fall back to autoload config
		local target = MacLib.AutoSave.Target
		if not target and isfile and isfile(MacLib.Folder .. "/settings/autoload.txt") then
			target = readfile(MacLib.Folder .. "/settings/autoload.txt")
		end

		if target then
			local ok, err = MacLib:SaveConfig(target)
		end
	end
	...
	function MacLib:LoadAutoLoadConfig()
		...
		if isfile(MacLib.Folder .. "/settings/autoload.txt") then
			...
			-- Pre-set AutoSave target BEFORE loading config so the toggle callback has it available
			if isfile(MacLib.Folder .. "/settings/autosave.txt") then
				MacLib.AutoSave.Target = readfile(MacLib.Folder .. "/settings/autosave.txt")
			else
				-- Default to the autoloaded config as the save target
				MacLib.AutoSave.Target = name
			end
	...
	function MacLib:LoadConfig(Path)
		...
		task.defer(function()
			MacLib.IsLoadingConfig = false
			MacLib.CurrentConfig = Path
			for _, hook in next, MacLib.OnConfigLoaded do
				task.spawn(hook, Path)
			end
		end)
...
function MacLib:Demo()
	local Window = MacLib:Window({
		...
		TabWidth = 0.325,-- Ratio of the left side (tab list) and the right side (content)
		DragStyle = 1,
...
local function AttackModule()
	...
	-- State: Normal Hit (Kill Aura)
	local _range = 999999
	local _delay = 0.1
	local _hitRunning = false
	local _hitConnection = nil
	local _hitTimer = 0
	local _hitTargetTypes = { Mob = true, Boss = true, Player = true }

	-- State: Auto Cast Skill
	local _skillDelay = 0.2
	local _skillRunning = false
	local _skillConnection = nil
	local _skillTimer = 0
	local _skillTargetTypes = {} -- mặc định không bật gì

	-- Skill list RIÊNG cho mỗi loại target
	-- Mỗi type có: skills = {1, 3, ...}, index = vị trí xoay vòng hiện tại
	local _perTypeSkills = {
		Mob    = { skills = {}, index = 1 },
		Boss   = { skills = {}, index = 1 },
		Player = { skills = {}, index = 1 },
	}

	-- Map tên phím → skill index
	local SKILL_MAP = { Z = 1, X = 2, C = 3, V = 4, F = 5 }

	-- Helper: convert UI dropdown value → sorted skill index list
	local function parseSkillNames(skillNames)
		local list = {}
		for name, _ in pairs(skillNames) do
			local idx = SKILL_MAP[name]
			if idx then
	...
	------------------------------------------------------------
	-- Tìm target GẦN NHẤT phù hợp với allowedTypes
	-- Trả về: model, targetType ("Mob"/"Boss"/"Player")
	------------------------------------------------------------
	local function getClosestTarget(allowedTypes)
		...
		-- 1) Quét NPCs folder (Mob + Boss)
		if allowedTypes["Mob"] or allowedTypes["Boss"] then
				...
				for _, obj in pairs(npcFolder:GetChildren()) do
					if obj:IsA("Model") and obj ~= myChar then
						...
						if root and hum and hum.Health > 0 then
							local targetType = string.find(obj.Name, "Boss") and "Boss" or "Mob"
							if allowedTypes[targetType] then
								local dist = (root.Position - myRoot.Position).Magnitude
								if dist <= _range and dist < closestDist then
									closestDist = dist
									closestModel = obj
		...
		return closestModel, closestType
	end
	...
	function self:SetDelay(delay)
		_delay = delay
	end

	function self:SetSkillDelay(delay)
		_skillDelay = delay
	end

	-- Đặt loại target cho Kill Aura
	function self:SetHitTargetTypes(types)
		_hitTargetTypes = types or {}
	end

	-- Đặt loại target cho Skill Cast
	function self:SetSkillTargetTypes(types)
		_skillTargetTypes = types or {}
	end

	-- Đặt skill list cho MỘT loại target cụ thể
	-- typeName: "Mob" / "Boss" / "Player"
	-- skillNames: {Z = true, C = true} (từ UI multi-dropdown)
	function self:SetTypeSkills(typeName, skillNames)
		if _perTypeSkills[typeName] then
			_perTypeSkills[typeName].skills = parseSkillNames(skillNames)
			_perTypeSkills[typeName].index = 1
		end
	end

	------------------------------------------------------------
	-- Kill Aura: Auto Normal Hit
	------------------------------------------------------------
	function self:Start()
		if _hitRunning then return end
		_hitRunning = true
		_hitTimer = 0

		_hitConnection = RunService.Heartbeat:Connect(function(dt)
			if not _hitRunning then return end

			_hitTimer = _hitTimer + dt
			if _hitTimer < _delay then return end
			_hitTimer = 0

			local target = getClosestTarget(_hitTargetTypes)
			if target then
				local root = target:FindFirstChild("HumanoidRootPart")
				if root then
					RequestHit:FireServer(root.Position)
				end
			end
		end)

		print("✅ Kill Aura đã BẬT!")
	end

	function self:Stop()
		_hitRunning = false
		if _hitConnection then
			_hitConnection:Disconnect()
			_hitConnection = nil
		end
	...
	------------------------------------------------------------
	-- Auto Cast Skill
	-- Tìm target gần nhất → lấy type → dùng skill list của type đó
	------------------------------------------------------------
	function self:StartSkills()
		if _skillRunning then return end
		_skillRunning = true
		_skillTimer = 0

		_skillConnection = RunService.Heartbeat:Connect(function(dt)
			if not _skillRunning then return end

			_skillTimer = _skillTimer + dt
			if _skillTimer < _skillDelay then return end
			_skillTimer = 0

			local target, targetType = getClosestTarget(_skillTargetTypes)
			if not target or not targetType then return end

			-- Lấy skill list cho đúng loại target
			local typeData = _perTypeSkills[targetType]
			if not typeData or #typeData.skills == 0 then return end

			local root = target:FindFirstChild("HumanoidRootPart")
			if not root then return end

			local skillId = typeData.skills[typeData.index]
			RequestAbility:FireServer(skillId, {
				["TargetName"] = target.Name,
				["TargetPosition"] = root.Position,
				["Variant"] = 1,
			})

			-- Xoay vòng index cho type này
			typeData.index = (typeData.index % #typeData.skills) + 1
		end)

		print("✅ Auto Cast Skill đã BẬT!")
	end

	function self:StopSkills()
		_skillRunning = false
		if _skillConnection then
			_skillConnection:Disconnect()
			_skillConnection = nil
		end
		-- Reset tất cả indices
		for _, data in pairs(_perTypeSkills) do
			data.index = 1
		end
		print("⛔ Auto Cast Skill đã TẮT!")
	end
